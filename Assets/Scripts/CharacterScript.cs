...
public class CharacterScript : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (isPlayer)
        {
            ...
            {
                if (currentState == (int)STATE.IDLE || currentState == (int)STATE.WALKING)
                {
                    PerformAttack("Attack", jabDamage, 0.4f, 1);
                }
            }
            ...
            {
                bool canCombo = (currentState == (int)STATE.ATTACKING && attackLevel == 1);
                if (currentState == (int)STATE.IDLE || currentState == (int)STATE.WALKING || canCombo)
                {
                    PerformAttack("Kick", kickDamage, 0.5f, 2);
                }
            }

            // Level 3: Specials (I, O, P)
            bool inputSpecial = Input.GetKeyDown(KeyCode.I) || Input.GetKeyDown(KeyCode.O) || Input.GetKeyDown(KeyCode.P);
            if (inputSpecial)
            {
                bool canCombo = (currentState == (int)STATE.ATTACKING && attackLevel < 3);
                if (currentState == (int)STATE.IDLE || currentState == (int)STATE.WALKING || canCombo)
                {
                    // Meter 1 (Blue)
                    if (Input.GetKeyDown(KeyCode.I) && myStats.TrySpendMeter(100f)) 
                        PerformSuperMove("Meter1", 1.5f);

                    // Meter 2 (Red)
                    if (Input.GetKeyDown(KeyCode.O) && myStats.TrySpendMeter(200f)) 
                        PerformSuperMove("Meter2", 2.0f);

                    // Meter 3 (Purple)
                    if (Input.GetKeyDown(KeyCode.P) && myStats.TrySpendMeter(300f)) 
                        PerformSuperMove("Meter3", 3.0f);
                }
            }
        }
        else
    ...
    void PerformAttack(string triggerName, float dmg, float duration, int level)
    {
    ...
    void PerformSuperMove(string triggerName, float duration)
    {
        velocity.x = 0;
        isAttacking = true;
