...
public class SukunaSummon : MonoBehaviour
{
    ...
    [Tooltip("How long the summon stays before fading away (if applicable)")]
    public float lifeSpan = 16.6f; 
    ...
    [Header("Effects")]
    public GameObject lightningPrefab; // For Nue

    private bool isFacingRight = true;
    private Rigidbody2D rb;
    private bool isFadingOut = false; // Flag to stop logic during fade
    ...
    void Start()
    {
        ...
        // Start Lifespan Timer for persistent summons (Mahoraga & Agito)
        if (startStateID == 1501 || startStateID == 1901)
        {
    ...
    void Update()
    {
        // Don't flip if fading out
        if (isFadingOut) return;

        if (target != null && Mathf.Abs(rb.linearVelocity.x) < 1f && rb.gravityScale == 0)
        {
            float dist = target.position.x - transform.position.x;
    ...
    IEnumerator LifespanRoutine()
    {
        ...
        // Time is up! Trigger the fade out logic.
        isFadingOut = true;
        StopAllCoroutines(); // Stop AI/Movement logic
    ...
    IEnumerator FadeOutSequence()
    {
        ...
        if (startStateID == 1501) // Mahoraga
        {
            // Specials.txt: State 1550 -> Anim 2006
            PlayAnimation("Action_2006");
            yield return new WaitForSeconds(GetAnimLength("Action_2006"));
        }
        else if (startStateID == 1901) // Agito
        {
            // Specials.txt: State 1950 -> Anim 1550
            PlayAnimation("Action_1550");
            yield return new WaitForSeconds(GetAnimLength("Action_1550"));
        }
    ...
    IEnumerator MugenStateLogic(int stateID)
    {
        ...
        switch (stateID)
        {
            ...
            case 1510: // Idle
                ...
                if (target == null || isFadingOut) yield break;
                ...
                else
                {
                    ...
                    if (rng == 0) StartCoroutine(MugenStateLogic(1540)); // A
                    else if (rng == 1) StartCoroutine(MugenStateLogic(1541)); // B
                    else if (rng == 2) StartCoroutine(MugenStateLogic(1542)); // C
                    else StartCoroutine(MugenStateLogic(1543)); // D
                }
                break;
            ...
            case 1520: // Move
                ...
                float walkTimer = 0f;
                while (walkTimer < 1.5f && Mathf.Abs(target.position.x - transform.position.x) > attackRange && !isFadingOut)
                {
                ...
                rb.linearVelocity = Vector2.zero;
                if(!isFadingOut) StartCoroutine(MugenStateLogic(1510));
                break;

            // Attacks
            case 1540: yield return PerformAttack("Action_2005"); if(!isFadingOut) StartCoroutine(MugenStateLogic(1510)); break;
            case 1541: yield return PerformAttack("Action_2105"); if(!isFadingOut) StartCoroutine(MugenStateLogic(1510)); break;
            case 1542: yield return PerformAttack("Action_2205"); if(!isFadingOut) StartCoroutine(MugenStateLogic(1510)); break;
            case 1543: yield return PerformAttack("Action_2305"); if(!isFadingOut) StartCoroutine(MugenStateLogic(1510)); break;
            ...
            case 1920: // Move
                ...
                float agitoTimer = 0f;
                while (agitoTimer < 1.0f && Mathf.Abs(target.position.x - transform.position.x) > attackRange && !isFadingOut)
                {
                    float dir = (target.position.x > transform.position.x) ? 1f : -1f;
                    rb.linearVelocity = new Vector2(dir * patrolSpeed, rb.linearVelocity.y);
                ...
                rb.linearVelocity = Vector2.zero;
                if(!isFadingOut) StartCoroutine(MugenStateLogic(1910));
                break;

            // Attacks
            case 1940: yield return PerformAttack("Action_1540"); if(!isFadingOut) StartCoroutine(MugenStateLogic(1910)); break;
            case 1941: yield return PerformAttack("Action_1541"); if(!isFadingOut) StartCoroutine(MugenStateLogic(1910)); break;
            case 1942: yield return PerformAttack("Action_1542"); if(!isFadingOut) StartCoroutine(MugenStateLogic(1910)); break;
            case 1943: yield return PerformAttack("Action_1543"); if(!isFadingOut) StartCoroutine(MugenStateLogic(1910)); break;
